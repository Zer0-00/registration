...
parser.add_argument('--lambda_KL', type=float, dest='weight_KL', default=10,
                    help='weight of KL loss for VAE(default: 50)')
args = parser.parse_args()
model_type = args.model_type

comment = model_type + 'KL:' + str(args.weight_KL) + 'lambda:' + str(args.weight) + 'latent_space:' + str(
    args.latent_space)
...
if args.load_model:
    ...
    model = vxm.networks.VxmTri.load(args.load_model, device)
else:
    # otherwise configure new model
    model = vxm.networks.VxmTri(
        inshape=inshape,
...
losses = [image_loss_func, image_loss_func, image_loss_func]
weights = [1, 0.5, 0.5]
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    for imgs1, imgs2, imgs3 in generator:
        ...
        for img in [inputs[:,i].unsqueeze(1) for i in range(3)]:
            curr_loss += (image_loss_func(outputs[-2], img) * weights[0] / 6 +
                          image_loss_func(outputs[-1], img) * weights[0] / 6)
            loss_list[-1] += curr_loss.item()
            loss += curr_loss
    ...
    with torch.no_grad():
        ...
        for img in [inputs[:,i].unsqueeze(1) for i in range(3)]:
            curr_loss += (image_loss_func(img, outputs[-2]) * weights[0] / 6 +
                          image_loss_func(img, outputs[-2]) * weights[0] / 6)
            loss_list[-1] += curr_loss.item()
            loss += curr_loss
    ...
    defos = outputs[3].to('cpu').permute(0, 2, 3, 4, 1).numpy()
    J = []
    below_zero = []
    for defo in defos:
    ...
    writer.add_scalars(args.image_loss,
                       {'train': means_loss.sum() - means_loss[3], 'val': means_loss_val.sum() - means_loss_val[3]},
                       epoch)
    writer.add_scalars('gradient of Defomation',
                       {'train': means_loss[3], 'val': means_loss_val[3]}, epoch)
